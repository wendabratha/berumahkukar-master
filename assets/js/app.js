var config = {
  geojson: "http://geoportal.kukarkab.go.id:8080/geoserver/pgsql/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=pgsql%3Abedah_rumah&maxFeatures=37000&outputFormat=application%2Fjson",
  
  title: "BeRUMAH-IDAMAN",
  layerName: "Sasaran Bedah Rumah",
  hoverProperty: "kecamatan",
  sortProperty: "kecamatan",
  sortOrder: "desc"
};
...
function drawCharts() {
  // Status
  $(function() {
    var result = alasql("SELECT status_sasaran AS label, COUNT(*) AS total FROM ? GROUP BY status_sasaran", [features]);
    ...
    // Sumber Data
  $(function() {
    var result = alasql("SELECT sumber_data AS label, COUNT(*) AS total FROM ? GROUP BY sumber_data", [features]);
    var columns = $.map(result, function(sumberdata) {
      return [[sumberdata.label, sumberdata.total]];
    });
    var chart = c3.generate({
        bindto: "#sumberdata-chart",
        data: {
          type: "bar",
          columns: columns
        }
  ...
  // Zones
  // $(function() {
  //   var result = alasql("SELECT pekerjaan AS label, COUNT(*) AS total FROM ? GROUP BY pekerjaan", [features]);
  ...
  // tjsp
  $(function() {
    var result = alasql("SELECT penyasar AS label, COUNT(*) AS total FROM ? GROUP BY penyasar", [features]);
    var columns = $.map(result, function(status) {
      return [[status.label, status.total]];
    });
    var chart = c3.generate({
